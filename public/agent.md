...
2. Produce a Miuix Canvas document JSON (`version: 3`) with `title`, optional `brief` (one-line app purpose for the coding prompt), `platform` (`cmp` | `android` | `web`), `theme` (`mode`, `seed`, `monet`; `monet` defaults to `false` so the sketch uses official HyperOS Light/Dark, not Material Monet) and `screens`.
