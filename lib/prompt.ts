import { KIND_TEXT, PLATFORM_TEXT, TRANSITION_TEXT, type Lang } from "./i18n";
import { KIND_SPEC } from "./tokens";
import type { Doc, Item, Platform, Screen } from "./types";
import { BACK_TARGET, frameSize } from "./types";
...
function themeLine(doc: Doc, lang: Lang): string {
  ...
  if (lang === "zh") {
    return `主题：${mode}，种子色 ${seed}${doc.theme.monet ? "，用 ThemeController(ColorSchemeMode.MonetSystem, keyColor = Color(0xFF" + seed.slice(1) + ")) 开启 Monet 动态取色" : "，用 lightColorScheme / darkColorScheme"}。背景用 surface（浅色约 #F7F7F7），卡片与设置项用 surfaceContainer，主色用于 Button / Switch / Slider。圆角按 Miuix 默认（按钮与卡片 16.dp，超椭圆 squircle）。`;
  }
  return `Theme: ${mode}, seed ${seed}${doc.theme.monet ? `, with ThemeController(ColorSchemeMode.MonetSystem, keyColor = Color(0xFF${seed.slice(1)}))` : ", using lightColorScheme / darkColorScheme"}. Background is surface (~#F7F7F7 in light). Cards and preferences sit on surfaceContainer. Primary tints Button, Switch and Slider. Use Miuix default 16.dp squircle corners.`;
}
...
function describeItem(doc: Doc, it: Item, lang: Lang): string {
  ...
  if (it.tabs?.length) {
    const names = it.tabs.map((t) => t.label || t.icon).join(lang === "zh" ? "、" : ", ");
...
function describeScreen(doc: Doc, screen: Screen, lang: Lang): string {
  ...
  const size = screen.preset === "desktop"
    ? (lang === "zh" ? `桌面 ${w}×${h}dp` : `desktop ${w}×${h}dp`)
  ...
  const lines = sorted.map((it) => describeItem(doc, it, lang));
  return `${head}${note}\n${lines.join("\n")}`;
}
...
export function buildPrompt(doc: Doc, lang: Lang, onlyScreenId?: string): string {
  ...
  const intro = lang === "zh"
    ? `做一款名为「${title}」的应用。${STACK[doc.platform][lang]}界面必须使用 Miuix（https://github.com/compose-miuix-ui/miuix）组件，不要用 Material 3。HyperOS 风格：灰底白卡片、16.dp 超椭圆圆角、设置项用 Preference。`
    : `Build an app named “${title}”. ${STACK[doc.platform][lang]} The UI must use Miuix (https://github.com/compose-miuix-ui/miuix) components, not Material 3. HyperOS look: gray background, white cards, 16.dp squircle corners, settings rows as Preference items.`;

  const nav = lang === "zh"
    ? "用 Scaffold + TopAppBar / NavigationBar（桌面宽度改 NavigationRail）。屏幕之间用 miuix-nav 或普通导航；标明「返回」的走返回栈。"
    : "Use Scaffold with TopAppBar / NavigationBar (NavigationRail on desktop widths). Navigate with miuix-nav or a regular back stack.";

  const body = screens.map((s) => describeScreen(doc, s, lang)).join("\n\n");
  const footer = lang === "zh"
    ? `实现提示：设置项放在 Card 里分组；列表垂直排列，左右边距 16.dp。保持 Miuix 的默认间距与排版。目标平台：${PLATFORM_TEXT[lang][doc.platform]}。`
    : `Implementation notes: group preference rows inside Card. Vertical list, 16.dp side margins. Keep Miuix default spacing and type. Target: ${PLATFORM_TEXT[lang][doc.platform]}.`;

  return [intro, themeLine(doc, lang), nav, body, footer].join("\n\n");
}
