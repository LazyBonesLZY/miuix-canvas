...
export const KIND_SPEC: Record<Kind, KindSpec> = {
  ...
  floatingToolbar: { kind: "floatingToolbar", category: "actions", icon: "construction", w: () => 220, h: 56, defaultLabel: L("工具", "Tools", "ツール", "도구"), tabs: [{ icon: "edit", label: "" }, { icon: "content_copy", label: "" }, { icon: "delete", label: "" }], composable: "FloatingToolbar" },
  ...
  smallTitle: { kind: "smallTitle", category: "navigation", icon: "title", w: full, h: 40, defaultLabel: L("常用功能", "Shortcuts", "ショートカット", "바로가기"), composable: "SmallTitle" },
  ...
  floatingNav: { kind: "floatingNav", category: "navigation", icon: "dock_to_bottom", w: () => 280, h: 88, variants: ["default", "iosLike"], defaultLabel: L("悬浮导航", "Floating nav", "フローティングナビ", "플로팅 탐색"), tabs: [{ icon: "home", label: "首页" }, { icon: "explore", label: "发现" }, { icon: "person", label: "我的" }], composable: "FloatingNavigationBar" },
  ...
  tooltip: { kind: "tooltip", category: "containment", icon: "tooltip_2", w: () => 140, h: 40, variants: ["plain", "rich"], defaultLabel: L("提示文字", "Tooltip", "ツールチップ", "툴팁"), defaultSupporting: L("补充说明", "More detail", "補足", "추가 설명"), composable: "Tooltip" },
  textField: { kind: "textField", category: "inputs", icon: "edit_note", w: full, h: 64, defaultLabel: L("输入内容", "Enter text", "入力", "입력"), composable: "TextField" },
  ...
  numberPicker: { kind: "numberPicker", category: "inputs", icon: "pin", w: () => 160, h: 225, defaultLabel: L("12", "12", "12", "12"), composable: "NumberPicker" },
  ...
  text: { kind: "text", category: "content", icon: "notes", w: full, h: 28, defaultLabel: L("正文", "Body", "本文", "본문"), composable: "Text" },
  image: { kind: "image", category: "content", icon: "image", w: full, h: 160, defaultLabel: L("", "", "", ""), composable: "Image" },
  badge: { kind: "badge", category: "content", icon: "mark_chat_unread", w: () => 24, h: 16, variants: ["number", "dot"], defaultLabel: L("3", "3", "3", "3"), composable: "Badge" },
  ...
  basicPref: { kind: "basicPref", category: "preference", icon: "view_agenda", w: full, h: 80, defaultLabel: L("通用项", "Basic row", "基本項目", "기본 항목"), defaultSupporting: L("自定义右侧内容", "Custom trailing slot", "末尾スロットをカスタム", "오른쪽 슬롯 사용자화"), composable: "BasicComponent" },
  switchPref: { kind: "switchPref", category: "preference", icon: "toggle_on", w: full, h: 80, defaultLabel: L("深色模式", "Dark mode", "ダークモード", "다크 모드"), defaultSupporting: L("跟随系统", "Follow system", "システムに合わせる", "시스템 따름"), checked: true, composable: "SwitchPreference" },
  checkboxPref: { kind: "checkboxPref", category: "preference", icon: "check_box", w: full, h: 80, defaultLabel: L("同步数据", "Sync data", "データを同期", "데이터 동기화"), defaultSupporting: L("使用移动网络时同步", "Sync on mobile data", "モバイル通信でも同期", "모바일 데이터에서도 동기화"), checked: false, composable: "CheckboxPreference" },
  ...
  sliderPref: { kind: "sliderPref", category: "preference", icon: "tune", w: full, h: 96, defaultLabel: L("字体大小", "Font size", "文字サイズ", "글자 크기"), value: 0.5, composable: "SliderPreference" },
  rangeSliderPref: { kind: "rangeSliderPref", category: "preference", icon: "linear_scale", w: full, h: 96, defaultLabel: L("音量范围", "Volume range", "音量レンジ", "음량 범위"), value: 0.8, from: 0.2, composable: "RangeSliderPreference" },
  dropdownPref: { kind: "dropdownPref", category: "preference", icon: "arrow_drop_down_circle", w: full, h: 80, variants: ["window", "overlay"], defaultLabel: L("语言", "Language", "言語", "언어"), defaultSupporting: L("简体中文", "English", "日本語", "한국어"), tabs: [{ icon: "", label: "中文" }, { icon: "", label: "英文" }, { icon: "", label: "日文" }], composable: "WindowDropdownPreference" },
  spinnerPref: { kind: "spinnerPref", category: "preference", icon: "tune", w: full, h: 80, variants: ["window", "overlay"], defaultLabel: L("排序方式", "Sort by", "並べ替え", "정렬"), defaultSupporting: L("名称", "Name", "名前", "이름"), tabs: [{ icon: "", label: "名称" }, { icon: "", label: "日期" }, { icon: "", label: "大小" }], composable: "WindowSpinnerPreference" },
  arrowPref: { kind: "arrowPref", category: "preference", icon: "chevron_right", w: full, h: 80, defaultLabel: L("关于本机", "About phone", "端末情報", "휴대전화 정보"), defaultSupporting: L("HyperOS 2", "HyperOS 2", "HyperOS 2", "HyperOS 2"), composable: "ArrowPreference" },
};

/** Official wrap-content minimums. Preference rows need 16+16 padding plus title/summary/switch. */
export function officialMinHeight(kind: Kind, supporting?: string): number {
  if (kind === "sliderPref" || kind === "rangeSliderPref") return 96;
  if (
    kind === "basicPref" ||
    kind === "switchPref" ||
    kind === "checkboxPref" ||
    ...
    return supporting ? 80 : 56;
  }
  if (kind === "smallTitle") return 40;
  if (kind === "text") return 28;
  if (kind === "switch") return 28;
  if (kind === "button") return 50;
  if (kind === "textField") return 64;
  if (kind === "numberPicker") return 225;
  if (kind === "snackbar") return 56;
  if (kind === "floatingToolbar") return 56;
  if (kind === "floatingNav") return 88;
  if (kind === "tooltip") return 40;
  return KIND_SPEC[kind].h;
}
...
export function applyVariant(it: Item, variant: string): Partial<Item> {
  ...
  if (it.kind === "floatingNav") {
    return variant === "iosLike" || variant === "glass" ? { ...patch, x: 0, w: Math.max(it.w, PHONE_W), h: 100 } : { ...patch, w: 280, h: 88 };
  }
  if (it.kind === "navigationRail") {
    return variant === "expanded" ? { ...patch, w: 240 } : { ...patch, w: 80 };
  }
  if (it.kind === "topAppBar") {
    return variant === "large" ? { ...patch, h: 104 } : { ...patch, h: 72 };
  }
  if (it.kind === "progress") {
  ...
  if (it.kind === "badge") {
    return variant === "dot" ? { ...patch, w: 6, h: 6, label: "" } : { ...patch, w: 24, h: 16 };
  }
  if (it.kind === "tabRow") {
    return variant === "contour" ? { ...patch, h: 45 } : { ...patch, h: 42 };
  }
  if (it.kind === "tooltip") {
    return variant === "rich" ? { ...patch, w: Math.max(it.w, 168), h: Math.max(it.h, 88) } : { ...patch, h: 40 };
  }
...
export function defaultPosition(kind: Kind, preset: FramePreset, existing: Item[]) {
  ...
  if (kind === "floatingNav") return { x: Math.round((w - 280) / 2), y: h - 64 - MARGIN - 88 };
  if (kind === "snackbar") return { x: MARGIN, y: h - 64 - MARGIN - 56 };
